...
class BatchProgress {
  ...
  final bool running;
  const BatchProgress(
      {this.jobs = const [], this.completed = 0, this.running = false});
...
class BatchService extends StateNotifier<BatchProgress> {
  ...
  Future<void> processAll(List<String> inputs,
      {bool addShadow = false, String format = 'jpg', int size = 2000}) async {
    ...
    state = BatchProgress(
        jobs: List.unmodifiable(jobs), completed: jobs.length, running: false);
  }
