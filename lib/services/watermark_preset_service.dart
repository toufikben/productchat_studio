...
class WatermarkPresetService {
  ...
  List<WatermarkPreset> getAll() {
    return Hive.box<dynamic>(_boxName)
        .values
        .map(
            (e) => WatermarkPreset.fromMap(Map<String, dynamic>.from(e as Map)))
        .toList();
  }
  ...
  Future<void> update(WatermarkPreset preset) async {
    ...
    for (final key in box.keys) {
      final existing = WatermarkPreset.fromMap(
  ...
  Future<void> remove(String id) async {
    ...
    final keys = box.keys.where((k) {
      final m =
          WatermarkPreset.fromMap(Map<String, dynamic>.from(box.get(k) as Map));
      return m.id == id;
    }).toList();
    for (final k in keys) {
      await box.delete(k);
  ...
  Future<String> apply(String imagePath, WatermarkPreset preset) async {
    ...
    if (preset.type == 'logo' && preset.logoPath != null) {
      ...
      if (logo != null) {
        ...
        final (x, y) =
            _position(image.width, image.height, logoW, logoH, preset.position);
      ...
      final (x, y) = _position(
          image.width,
          image.height,
          preset.text!.length * preset.fontSize ~/ 2,
          bandHeight,
          preset.position);

      for (var dy = 0; dy < bandHeight; dy++) {
        for (var dx = 0;
            dx < preset.text!.length * preset.fontSize ~/ 2;
            dx++) {
          final px = x + dx;
          final py = y + dy;
  ...
  (int, int) _position(int w, int h, int wmW, int wmH, String pos) {
    switch (pos) {
      ...
      case 'bottomLeft':
        return (16, h - wmH - 16);
      case 'center':
        return ((w - wmW) ~/ 2, (h - wmH) ~/ 2);
      default:
        return (w - wmW - 16, h - wmH - 16);
    }
  }
}
