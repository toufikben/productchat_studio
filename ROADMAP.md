...
### قرار Qwen المؤقت — 2026-09-15

- [ ] **ليس هذا حلاً أمنياً نهائياً:** يمكن استخدام `QWEN_API_KEY` مؤقتاً عبر `--dart-define` في نسخ التطوير أو الاختبار فقط، من دون حفظه في Git أو Hugging Face.
- [ ] **لا نضيف تشفيراً وهمياً للمفتاح داخل APK:** تشفير المفتاح داخل التطبيق مع وجود كود فكّه لا يمنع استخراجه؛ لذلك لا يُعتبر بديلاً عن Backend.
- [ ] **عند توفر الميزانية:** نقل المسار إلى `Flutter → Supabase Edge Function → WaveSpeed/Qwen`، ووضع المفتاح في Supabase Secret، مع Auth وrate limiting وحصة يومية وحذف الملفات المؤقتة.
- [ ] **قبل الإطلاق العام:** استخدام مفتاح إنتاج منفصل ومحدود الصلاحيات/الاستهلاك، وإزالة اتصال Qwen المباشر من التطبيق أو إخفاء الميزة حتى اكتمال Backend.

### قرار النماذج العملي — 2026-09-15

- [x] لا نستبدل Qwen حالياً بنموذج صغير وندّعي نفس الجودة؛ لا يوجد بديل on-device صغير يطابقه في التعديل المحادثي العام، وفهم التعليمات، وتعديل العناصر مع الحفاظ على أسلوب الصورة.
- [x] نعتمد `MI-GAN` لإزالة الخلفية، و`LaMa` للـ inpainting بالقناع، و`Real-ESRGAN` للتحسين ×4 كمسارات محلية متخصصة بعد إغلاق اختبارات Android.
- [ ] يمكن تقييم `MobileSAM` أو نموذج matting خفيف لاحقاً لتحسين اختيار الكائن/إنشاء القناع، لكنهما مساعدان ولا يستبدلان Qwen.
- [ ] **مؤجل بسبب التكلفة:** Qwen/WaveSpeed ليس ضمن الإصدار العام الحالي؛ يبقى مساراً مستقبلياً للتعديل المحادثي العام، وليس بديلاً للوظائف المحلية المتخصصة.
- [ ] **عند توفر الميزانية:** إضافة Backend، وضع المفتاح في Secret، نظام Credits، حد يومي، وخصم الرصيد بعد نجاح العملية فقط، مع معالجة الفشل والمهلة وإعادة المحاولة.
- [x] **قرار النسخة الأولى:** إطلاق الوظائف المحلية المتخصصة بدلاً من إضافة نموذج صغير أقل جودة باسم مضلل أو كشف مفتاح API داخل APK.
...
- [x] استبعاد `qwen_edit_int8.onnx`؛ المستودع الرسمي يوفّر Qwen بصيغة Safetensors وليس ONNX، لذلك لم يُرفع ملف غير متوافق أو يُعاد تسميته.
- [x] استبدال التنفيذ المحلي في `QwenEditService` بواجهة WaveSpeed API مع مفتاح يمرر وقت البناء عبر `--dart-define=QWEN_API_KEY=...`.
- [x] حذف Qwen من `ModelManager` ومن ثوابت النماذج المحلية.
