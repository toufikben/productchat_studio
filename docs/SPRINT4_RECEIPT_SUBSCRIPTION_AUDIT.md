...
| RV-02 | High | Consumable Credits are granted by the client before trusted verification. | `BillingService._handlePurchases()` calls `ledger.addOnce()` for a recognized consumable `purchaseID` on `purchased`; restored consumables are rejected after the hardening change. | A local ledger protects only the current installation/storage state; it is not a cross-device or server-authoritative ledger. |
