...
The Free path now applies `FreeWatermarkService` to successful PatchMatch output before consuming quota. Editor shows explicit Pro labels and surfaces gate errors; Chat exposes a direct PatchMatch action and displays Free/Pro status. Reproducible image fixtures live under `test/fixtures/patchmatch/` and are regenerated by `tooling/generate_patchmatch_fixtures.py`. Flutter/Dart execution and Android instrumentation remain pending in the current environment.
